...
with tab2:
    ...
    if st.button("🔄 최신 데이터 불러오기"):
        try:
            ...
            if not df.empty:
                # 1. 숫자 컬럼을 명시적으로 숫자로 변환 (합계용)
                for col in ['price_suggested', 'price_confirmed', 'price_paid']:
                    if col in df.columns:
                        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(int)
                
                # 2. 합계 먼저 계산
                total_paid = df['price_paid'].sum()
                
                # 3. [핵심] 출력용 데이터를 '글자'로 100% 변환
                # f-string을 사용하여 콤마를 찍은 문자열로 변환합니다.
                df['price_suggested'] = df['price_suggested'].apply(lambda x: f"{x:,}")
                df['price_confirmed'] = df['price_confirmed'].apply(lambda x: f"{x:,}")
                df['price_paid'] = df['price_paid'].apply(lambda x: f"{x:,}")
                
                # 4. 컬럼 한글화
                df.columns = ['ID', '시간', '지점', '환자명', '구분', '상담항목', '경로', '상담자', '결과', '상담금액', '확정금액', '수납금액', '내용', '예약여부']
                
                # 5. 에러를 일으키는 st.dataframe 대신 st.table 또는 단순 출력 사용
                st.table(df) # table은 자동 포맷팅 시도를 하지 않아 안전합니다.
                
                st.divider()
                st.metric("총 수납 금액 합계", f"{total_paid:,}원")
            else:
