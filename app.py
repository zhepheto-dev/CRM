...
if branch != "지점을 선택하세요":
    ...
    with col2:
        st.subheader("💰 금액 정보")
        result_status = st.selectbox("결과", ["확정", "미확정", "보류", "상담없음"])
        
        def get_num(val):
            clean = re.sub(r'[^0-9]', '', str(val))
            return int(clean) if clean else 0

        # 수정 포인트: value를 ""로 설정하여 0이 미리 보이지 않게 했습니다.
        sug_raw = st.text_input("제시액", value="", placeholder="숫자만 입력")
        conf_raw = st.text_input("확정액", value="", placeholder="숫자만 입력")
        paid_raw = st.text_input("수납액", value="", placeholder="숫자만 입력")
